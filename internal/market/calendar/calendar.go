...
func GetMarketPhase(venue string, t time.Time, isHoliday bool) string {
	...
	if venue == "KRX" {
		...
		jstTime := t.In(kst.Location) // KST and JST are both UTC+9
...
func CapTimeAt1530(t time.Time) time.Time {
	kstTime := t.In(kst.Location)
	kst330 := time.Date(kstTime.Year(), kstTime.Month(), kstTime.Day(), 15, 30, 0, 0, kst.Location)
