...
var _ = Describe("Snapshot Render", func() {
	Context("Empty Snapshot", func() {
		It("shows all sections with n/a indicators", func() {
			s := &Snapshot{
				BusinessDate: "20260515", SessionStatus: "POST_CLOSE_UNRECONCILED", Timestamp: time.Date(2026, 5, 15, 15, 30, 0, 0, time.FixedZone("KST", 9*60*60)),
			...
			for _, want := range []string{
				"# Market Snapshot — 20260515 [POST_CLOSE_UNRECONCILED]",
				"## 1. 일중 가격 흐름",
				"## 2. 수급 (코스피)",
				"## 3. 시장 충격 지표",
				"## 4. 글로벌 참고 시세",
