...
func (f contributionFake) KOSPIIndexMarketCapSummary(context.Context, string) (*domesticstock.KOSPIMarketCapSummary, error) {
...
func (f contributionFake) InquirePrice(_ context.Context, code string) (*auth.RESTResponse, error) {
	...
	return &auth.RESTResponse{Body: map[string]any{"rt_cd": "0", "output": map[string]any{"prdy_ctrt": fts(change)}}}, nil
}
...
var _ = Describe("market structure collectors", func() {
	It("프로그램매매 최신 행을 선택하고 백만원을 억원으로 변환", func() {
		resp := &auth.RESTResponse{Body: map[string]any{"rt_cd": "0", "output": []any{
	...
	It("프로그램 집계시각 원문을 보존", func() {
		resp := &auth.RESTResponse{Body: map[string]any{"rt_cd": "0", "output": []any{map[string]any{
			"bsop_hour": "172100", "arbt_smtn_ntby_tr_pbmn": "0", "nabt_smtn_ntby_tr_pbmn": "100", "whol_smtn_ntby_tr_pbmn": "100",
		}}}}
		got, err := collectProgramTrade(context.Background(), programFake{resp}, "K")
		Expect(err).NotTo(HaveOccurred())
		Expect(got.AsOf).To(Equal("172100"))
	})

	It("KOSPI200 현물과 선물로 시장 베이시스를 교차 검증", func() {
		resp := &auth.RESTResponse{Body: map[string]any{"rt_cd": "0",
			"output1": map[string]any{"futs_prpr": "1300.15", "futs_prdy_clpr": "1381.40", "futs_prdy_ctrt": "-5.88", "mrkt_basis": "1.13"},
			"output3": map[string]any{"bstp_cls_code": "2001", "bstp_nmix_prpr": "1299.02", "bstp_nmix_prdy_ctrt": "-5.80"},
		}}
	...
	It("CB는 하락 방향만 계산하고 저점 기준 근접도를 보존", func() {
		idx := IndexLevel{BreadthOK: true, PrevClose: 8303.45, Price: 7844.28, Low: 7723.57, ChangePct: -5.53, OK: true}
		safety := buildMarketSafety(time.Now(), "20260702", idx, IndexLevel{BreadthOK: true}, IndexFutureSnapshot{}, IndexFutureSnapshot{}, nil)
		...
		idxPositive := IndexLevel{BreadthOK: true, PrevClose: 1000.0, Price: 1051.7, Low: 1020.0, ChangePct: 5.17, OK: true}
		safetyPos := buildMarketSafety(time.Now(), "20260702", idxPositive, IndexLevel{BreadthOK: true}, IndexFutureSnapshot{}, IndexFutureSnapshot{}, nil)
	...
	It("KOSDAQ 사이드카는 선물과 현물 조건을 같은 방향으로 모두 요구", func() {
		f := IndexFutureSnapshot{Code: "A06609", ChangePct: -6.2, SpotChangePct: -3.1, OK: true, SpotOK: true, Freshness: "FRESH"}
	...
	It("선물 급등 시 SIDECAR_SELL과 SIDECAR_BUY 임계 간격을 정확히 산출", func() {
		...
		k200 := IndexFutureSnapshot{Code: "A01609", ChangePct: 5.46, OK: true, Freshness: "FRESH"}
		safety := buildMarketSafety(now, "20260721", IndexLevel{BreadthOK: true}, IndexLevel{BreadthOK: true}, k200, IndexFutureSnapshot{}, nil)
	...
	It("전체 지수 후보 집합의 참고 비중을 계산하고 포인트는 미평가", func() {
		fake := contributionFake{
			summary: &domesticstock.KOSPIMarketCapSummary{BusinessDate: "20260702", Universe: domesticstock.KOSPIIndexMasterUniverse, WeightStatus: domesticstock.KOSPIIndexWeightStatus, TotalMarketCap: 600, Constituents: []domesticstock.KOSPIMarketCapConstituent{
				{Code: "005930", Name: "삼성전자", MarketCap: 400, KOSPIIndexMember: true, PreferredClass: "0"},
				{Code: "000660", Name: "SK하이닉스", MarketCap: 200, KOSPIIndexMember: true, PreferredClass: "0"},
			}},
			changes: map[string]float64{"005930": -5, "000660": -10},
		}
		got, err := collectContributions(context.Background(), fake, "20260702", IndexLevel{BreadthOK: true, PrevClose: 1000, OK: true})
		Expect(err).NotTo(HaveOccurred())
		Expect(got).To(HaveLen(2))
		Expect(got[0].WeightPct).To(BeNumerically("~", 100.0*400/600, 0.001))
		Expect(got[0].PointImpact).To(BeNil())
		Expect(got[1].PointImpact).To(BeNil())
	})
...
var _ = Describe("flow rates", func() {
	...
	It("분석 문구가 음수 수급을 순매도로 표시", func() {
		p := &Pulse{KOSPI: Market{
			Flow:        FlowSnapshot{EtcForeignOK: true, OK: true, Foreign: -5000},
	...
	It("렌더가 거래대금 화살표를 제거하고 no-save 경로와 미국채 bp를 표시", func() {
		...
		p := &Pulse{
			...
			KOSPI:    Market{Name: "KOSPI", Index: IndexLevel{BreadthOK: true, Price: 100, PrevClose: 105, ChangePct: -4.76, TradingValue: 12000, OK: true}},
			KOSDAQ:   Market{Name: "KOSDAQ"},
			USDKRW:   Window{ChangeOK: true, Symbol: "KRW=X", Label: "원/달러", Current: 1548.13, ChangePct: -0.17, Move1hPct: &usdkrwMove, OK: true},
			Macro:    []Window{{ChangeOK: true, Symbol: "^TNX", Label: "미국채10Y", Current: 4.5, ChangePct: 1, Move1hPct: &move1h, OK: true}},
		...
		Expect(out).To(ContainSubstring("기준대비 -0.17%"))
	...
	It("수급 렌더에 기타법인 및 기관 7개 세부항목(금융투자, 보험, 투신, 기타금융, 은행, 연기금, 사모)과 합계가 정상 표시", func() {
		p := &Pulse{
			...
			KOSPI: Market{
				Name: "KOSPI",
				Flow: FlowSnapshot{EtcForeignOK: true,
					Foreign:     -6219,
					Institution: -6427,
					Individual:  4682,
					EtcCorp:     7964,
	...
	It("시장폭, 단순스프레드(raw_spread), 환율 기준가, 사이드카 세부 등락률이 정상 렌더링", func() {
		p := &Pulse{
			...
			KOSPI: Market{
				Name: "KOSPI",
				Index: IndexLevel{BreadthOK: true,
					Price:      6759.44,
					PrevClose:  6788.0,
					ChangePct:  -0.43,
					Open:       6613.58,
			...
			KOSPI200Future: IndexFutureSnapshot{
				Code: "A01609", SpotOK: true, Alignment: "ALIGNMENT_UNVERIFIED",
				Price:         1062.70,
				SpotPrice:     1061.34,
				Basis:         1.36,
				RawSpread:     1.36,
		...
		Expect(out).To(ContainSubstring("상승 312 (상한 1) · 보합 50 · 하락 565 · 응답 분류합 928"))
		// 2. 단순스프레드(raw_spread) 명칭 검증
		Expect(out).To(ContainSubstring("단순스프레드(raw_spread) +1.36p (ALIGNMENT_UNVERIFIED)"))
		// 3. 환율 기준가 검증
